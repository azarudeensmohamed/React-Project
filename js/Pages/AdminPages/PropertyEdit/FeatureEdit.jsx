...
const FeatureEdit = ({setfeature,feature,selectedFeatures,setSelectedFeatures,handleSubmit}) => {
  ...
  const handleFeatureSelect = (featureId, checked) => {
    setSelectedFeatures((prevSelected) => {
      const safePrevSelected = Array.isArray(prevSelected) ? prevSelected : []; // Ensure it's an array

      let newSelected = safePrevSelected.filter((id) => id !== featureId); // Remove first to avoid duplicates

      if (checked) {
        newSelected.push(Number(featureId)); // Ensure values are numbers
      }
